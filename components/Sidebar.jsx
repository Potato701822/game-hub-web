  ...
  return (
    <aside className="w-72 bg-[#050505] border-r border-white/[0.03] flex flex-col fixed h-full z-50">
      <div className="p-12 mb-8">
        <div className="flex flex-col gap-0 cursor-pointer group" onClick={() => setView('inicio')}>
          <span className="text-white font-black text-4xl tracking-tighter italic leading-none group-hover:text-[#ff9d00] transition-all">KRAKEN</span>
      ...
      <nav className="flex-1 px-8 space-y-2">
        {menuItems.map(item => (
          <button 
            ...
            className={`w-full flex items-center gap-4 px-6 py-4 rounded-sm transition-all text-[10px] font-black uppercase tracking-[0.2em] relative ${activeView === item.id ? 'bg-white/[0.03] text-white' : 'text-zinc-600 hover:text-zinc-400'}`}
          >
            <item.icon size={18} className={activeView === item.id ? 'text-[#ff9d00]' : 'text-zinc-800'} />
            <span>{item.label}</span>
            {activeView === item.id && <div className="absolute left-0 w-[2px] h-6 bg-[#ff9d00]" />}
          </button>
        ))}
      </nav>

      <div className="p-8 border-t border-white/[0.03] bg-black/50">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <p className="text-[10px] font-black text-white uppercase tracking-wider">POTATO</p>
            <p className="text-[8px] text-emerald-500/80 font-black uppercase">Online</p>
          </div>
        </div>
      </div>
    </aside>

...
import { Home, LayoutGrid, Library, Download, User } from 'lucide-react';
      ...
      <nav className="flex-1 px-8 space-y-2">
        {menuItems.map(item => (
          <button key={item.id} onClick={() => setView(item.id)} className={`w-full flex items-center gap-4 px-6 py-4 rounded-sm transition-all text-[10px] font-black uppercase tracking-[0.2em] relative ${activeView === item.id ? 'bg-white/[0.03] text-white' : 'text-zinc-600 hover:text-zinc-400'}`}>
            ...
            {item.badge > 0 && <span className="ml-auto bg-[#ff9d00] text-black w-5 h-5 rounded-full flex items-center justify-center text-[9px]">{item.badge}</span>}
          </button>
        ))}
      </nav>
      <div className="p-8 border-t border-white/[0.03] flex items-center gap-4">
        <div className="w-10 h-10 rounded-full bg-zinc-900 flex items-center justify-center"><User size={20} /></div>
        <div><p className="text-[10px] font-black text-white uppercase">POTATO</p><p className="text-[8px] text-emerald-500 font-black uppercase">Online</p></div>
      </div>
    </aside>
  );
}
